...
> Select-to-drill for DeepSeek Harness (dsh): quote any selected chat text into the composer, or fork a drill session from the selected message — auto-archived when you leave, so the current conversation stays clean.
...
**dsh 会话选中钻取插件**：在会话历史里划选任意内容时浮现操作条，提供两条"针对性追问"路径——**引用到当前输入框**，或 **fork 出独立钻取会话**（离开时自动归档）。不污染当前会话，不弄乱侧边栏。
...
| **钻取** | 从**选中消息所在节点** `fork` 出子会话并自动切换；子会话改名「🔍 钻取 · <源标题>」，输入框预填 Drill 焦点引用块 |
| **自动归档** | 离开钻取会话时自动从侧边栏归档隐藏（running 时顺延到空闲，可在设置关闭） |
...
# 或从 GitHub 安装
dsh plugin --profile web add git+https://github.com/<you>/dsh-drill.git
...
```sh
dsh plugin --profile web remove dsh-drill
```
...
3. **钻取**：切到新会话「🔍 钻取 · <源标题>」——历史完整、上下文锚定在选中消息处，输入框已预填 Drill 焦点块，直接提问；
4. 从钻取会话离开时 → toast「已归档」，侧边栏无残留。
...
flowchart TD
    ...
    C -->|钻取| E["sessions.fork(atSeq = 节点 anchorSeq)"]
    E --> F[子会话改名 🔍 钻取 · …]
    F --> G[自动切换 + 预填 Drill 焦点块]
...
| 键 | 默认 | 说明 |
|----|------|------|
| `dsh-drill:autoArchive` | `"1"` | 离开钻取会话时自动归档；`"0"` 关闭 |
| `dsh-drill:truncateChars` | `"2000"` | 引用文本截断长度（≥100 生效） |
| `dsh-drill:activeDrills` | `[]` | 活跃钻取会话表（内部持久化，勿手改） |
...
- **fork 锚点**：`binding.session.getSnapshot().chat.nodes.get(key).anchorSeq` 定位选中节点；fork 后 `child.rename()` 钉住子会话标题（不被 LLM 自动再生）；
- **归档 watcher**：root 级订阅 `sessions.list`，当前选择离开"活跃钻取会话"且该会话不在 running 时调用 `workspaces.archiveSession`；
- hooks 纪律：任何 hook 都不条件调用（`useInput` 由内层子组件无条件调用）。

## 已知限制

- **归档是单向操作**（当前 dsh 版本没有 unarchive API）：归档后仅从侧边栏隐藏，会话日志仍在磁盘 `~/.dsh/sessions/` 下，数据不会丢；
- 正在运行的回合（streaming 尾部）不能作为 fork 锚点，此时钻取按钮会给出提示；
- 依赖 ui-conversation 的 `data-*` DOM 锚点：dsh 升级若改变这些属性，需同步本插件的选择器常量。

## FAQ

**Q: 我的原始会话标题看起来被加了后缀？**
A: 不是本插件。dsh 会自动用 LLM 根据你的第一句话生成会话标题，措辞纯属巧合。本插件只给钻取**子会话**改名（`🔍 钻取 · …`），从不修改源会话。
