...
//   3. btw：sessions.fork(选中节点 anchorSeq) → 自动切换到子会话并预填引用块；
//      离开该btw会话时自动 workspace.archiveSession（running 时顺延，设置可关）
//
// 只走公开面：
	...
	factory: (require) => {
		...
		const DICT = {
			zh: {
				...
				"btw.head": '> 🔖 btw 焦点（fork 自「{title}」）',
				"toast.archived": "btw会话已归档（不可逆）",
				"toast.archiveFailed": "btw会话归档失败",
				"toast.quoteUnavailable": "当前会话输入框不可用",
				"toast.quoteFailed": "引用写入失败",
				"btw.failed": "btw失败",
				"btw.titlePrefix": "btw",
				"btw.runningTail": "选区位于进行中的回合，暂不能从这里btw",
			},
			en: {
				...
				"btw.head": '> 🔖 btw focus (forked from "{title}")',
